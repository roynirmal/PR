% PR assignment 
function [errorTable] = classifiersErrorsFeaturessp3(resizeSize, resizeMethod, pr_ds_features, trn, tst, nrTrObjectsPerClass,thresholding,features, nrFeat,featselect,pca)
...
errorTable=cell(5,2);
class =['rbsvc' 'svc*fishercc' ' pksvc' 'svc'];
errors = [strcat( " nrTrObjectsPerClass:",string(nrTrObjectsPerClass)," resizeSize:",string(resizeSize)," resizeMethod:",string(resizeMethod)," thresholding:",string(thresholding)," nrFeatures:",string(nrFeat)," featSelect:",string(featselect)," pca:",string(pca), " pcadimension:", string(n)) e1 e2 e3 e4];
for i=1:5
