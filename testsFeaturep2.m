...
errors={' ' ;'svc' ;'qdc'; 'parzen' ; 'bpxnc'; 'loglc'; 'knnc'; 'fisherc'};
% 
nrData = [10, 200];
resizeSize =  16; 
resizeMethod =  {'bicubic'};
 ...
 nrFeat = [75, 100];
 featselect = {'featselp'};
pca = [false, 0.95, 0.9, 0.85, 0.8, 0.7] ;
          ...
          for k = 1:length(nrData)
                display('Trying a total of some objects');
                        for m = 1:length(nrFeat)
                          ...
                          for o = 1:length(pca)
                              eT =classifiersErrorsFeaturesp2(resizeSize, resizeMethod{1}, pr_ds_features, trn, tst,  nrData(k), false, true, nrFeat(m),featselect{1}, pca(o));
                              errors=[errors eT(:,2)];
                              
                          end
                      end

          filename = strcat('data',string(nrData(k)),'resizeSize',string(resizeSize),'resizeMethod', string(resizeMethod{1}), 'featureTrueThreshFale');
